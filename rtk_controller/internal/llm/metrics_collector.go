...
// WorkflowExecutionMetrics tracks workflow execution metrics
...
func (mc *MetricsCollector) RecordWorkflowExecution(workflowID string, duration time.Duration, success bool, intentAccuracy float64, errorReason string) {
	mc.workflowMutex.Lock()
	defer mc.workflowMutex.Unlock()

	// Update overall metrics
	mc.workflowMetrics.TotalExecutions++
	if success {
		mc.workflowMetrics.SuccessfulExecutions++
	} else {
		mc.workflowMetrics.FailedExecutions++
	}

	// Update timing metrics
	if mc.workflowMetrics.TotalExecutions == 1 {
		mc.workflowMetrics.AverageExecutionTime = duration
		mc.workflowMetrics.MinExecutionTime = duration
		mc.workflowMetrics.MaxExecutionTime = duration
	} else {
		// Rolling average
		mc.workflowMetrics.AverageExecutionTime = time.Duration(
			(int64(mc.workflowMetrics.AverageExecutionTime)*(mc.workflowMetrics.TotalExecutions-1) + int64(duration)) / mc.workflowMetrics.TotalExecutions,
		)
		if duration < mc.workflowMetrics.MinExecutionTime {
			mc.workflowMetrics.MinExecutionTime = duration
		}
		if duration > mc.workflowMetrics.MaxExecutionTime {
			mc.workflowMetrics.MaxExecutionTime = duration
		}
	}

	// Update intent accuracy (rolling average)
	if mc.workflowMetrics.TotalExecutions == 1 {
		mc.workflowMetrics.IntentAccuracy = intentAccuracy
	} else {
		mc.workflowMetrics.IntentAccuracy = (mc.workflowMetrics.IntentAccuracy*(float64(mc.workflowMetrics.TotalExecutions-1)) + intentAccuracy) / float64(mc.workflowMetrics.TotalExecutions)
	}

	// Update per-workflow stats
	if _, exists := mc.workflowMetrics.WorkflowStats[workflowID]; !exists {
		mc.workflowMetrics.WorkflowStats[workflowID] = &WorkflowStats{
			WorkflowID:      workflowID,
			FailureReasons:  make([]string, 0),
		}
	}

	stats := mc.workflowMetrics.WorkflowStats[workflowID]
	stats.ExecutionCount++
	stats.LastExecuted = time.Now()

	// Update success rate
	if success {
		stats.SuccessRate = (stats.SuccessRate*float64(stats.ExecutionCount-1) + 1.0) / float64(stats.ExecutionCount)
	} else {
		stats.SuccessRate = (stats.SuccessRate * float64(stats.ExecutionCount-1)) / float64(stats.ExecutionCount)
		if errorReason != "" {
			stats.FailureReasons = append(stats.FailureReasons, errorReason)
			// Keep only last 10 failure reasons
			if len(stats.FailureReasons) > 10 {
				stats.FailureReasons = stats.FailureReasons[len(stats.FailureReasons)-10:]
			}
		}
	}
...
func (mc *MetricsCollector) GetWorkflowMetrics() *WorkflowExecutionMetrics {
	mc.workflowMutex.RLock()
	defer mc.workflowMutex.RUnlock()

	// Return a deep copy
	stats := make(map[string]*WorkflowStats)
	for k, v := range mc.workflowMetrics.WorkflowStats {
		reasons := make([]string, len(v.FailureReasons))
		copy(reasons, v.FailureReasons)
		stats[k] = &WorkflowStats{
	...
	return &WorkflowExecutionMetrics{
		TotalExecutions:      mc.workflowMetrics.TotalExecutions,
		SuccessfulExecutions: mc.workflowMetrics.SuccessfulExecutions,
		FailedExecutions:     mc.workflowMetrics.FailedExecutions,
		AverageExecutionTime: mc.workflowMetrics.AverageExecutionTime,
		MinExecutionTime:     mc.workflowMetrics.MinExecutionTime,
		MaxExecutionTime:     mc.workflowMetrics.MaxExecutionTime,
		IntentAccuracy:       mc.workflowMetrics.IntentAccuracy,
		WorkflowStats:        stats,
		LastUpdated:          mc.workflowMetrics.LastUpdated,
	}
}
...
func (mc *MetricsCollector) Reset() {
	...
	mc.workflowMetrics = &WorkflowExecutionMetrics{
		WorkflowStats: make(map[string]*WorkflowStats),
		LastUpdated:   time.Now(),
	}
